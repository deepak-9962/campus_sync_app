...
class AttendanceService {
  ...
  Future<List<Map<String, dynamic>>> getAllStudentsAttendance({
    ...
    try {
      ...
      final attendanceResponse = await query.order('registration_no', ascending: true);
  ...
  Future<List<Map<String, dynamic>>> getAllStudents({
    ...
    try {
      ...
      final studentsResponse = await query.order('registration_no', ascending: true);
  ...
  // ============================================================
  // ATTENDANCE EDITING METHODS - For Staff, Admin, HOD
  // ============================================================

  /// Update a single attendance record's status
  /// Supports: 'present', 'absent', 'late'
  Future<bool> updateAttendanceStatus({
    ...
    try {
      await _supabase.from('attendance').update({
        ...
        'updated_by': updatedBy ?? _supabase.auth.currentUser?.id,
      }).eq('id', attendanceId);

      print('Successfully updated attendance $attendanceId to $newStatus');
      return true;
    } catch (e) {
  ...
  /// Update daily attendance record's status
  Future<bool> updateDailyAttendanceStatus({
    ...
    try {
      await _supabase.from('daily_attendance').update({
        'status': newStatus,
        'remarks': remarks ?? 'Status updated to $newStatus',
        'updated_at': DateTime.now().toIso8601String(),
        'updated_by': updatedBy ?? _supabase.auth.currentUser?.id,
      ...
      print('Successfully updated daily attendance $recordId to $newStatus');
      return true;
    } catch (e) {
      print('Error updating daily attendance status: $e');
      return false;
    }
  }

  /// Bulk update multiple attendance records to 'late' status
  Future<bool> markStudentsAsLate({
    ...
    try {
      ...
      print('Successfully marked ${studentIds.length} students as late');
      return true;
    } catch (e) {
      print('Error marking students as late: $e');
      return false;
    }
  }

  /// Get all absent students for a specific period (for marking late arrivals)
  Future<List<Map<String, dynamic>>> getAbsentStudentsForPeriod({
    required String date,
    required String period,
    required String department,
    required String semester,
    ...
    try {
      final response = await _supabase
          ...
          .select('''
            ...
            student_id,
            status,
            students(id, name, roll_number, registration_number)
          ''')
          .eq('date', date)
          .eq('period', period)
          .eq('department', department)
  ...
  /// Get attendance records for editing (all statuses)
  Future<List<Map<String, dynamic>>> getAttendanceForEditing({
    required String date,
    required String period,
    required String department,
    required String semester,
    ...
    try {
      ...
      print('Error fetching attendance for editing: $e');
      return [];
    }
  }

  /// Get daily attendance records for editing
  Future<List<Map<String, dynamic>>> getDailyAttendanceForEditing({
    required String date,
    required String department,
    required String semester,
    required String section,
    ...
    try {
      ...
      print('Error fetching daily attendance for editing: $e');
      return [];
    }
  }

  /// Check if user has permission to edit attendance
  Future<bool> canEditAttendance(String userId) async {
    try {
      final response = await _supabase
          .from('users')
          .select('role')
      ...
      // Only staff, admin, and hod can edit attendance
      return role == 'staff' || role == 'admin' || role == 'hod' || role == 'faculty';
    } catch (e) {
  ...
  /// Get attendance edit history for a record
  Future<List<Map<String, dynamic>>> getAttendanceEditHistory({
    required String attendanceId,
  }) async {
    try {
