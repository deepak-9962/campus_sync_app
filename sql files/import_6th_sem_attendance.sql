-- ============================================================================
-- ATTENDANCE DATA IMPORT SCRIPT FOR 6TH SEMESTER CSE-A
-- ============================================================================
-- This script imports 15 days of attendance data (Dec 4-20, 2025)
-- for 6th semester CSE Section A students
-- 
-- IMPORTANT: Before running this script:
-- 1. Make sure a subject exists for 6th semester CSE (e.g., 'CS601' or similar)
-- 2. Verify students exist in the students table with matching registration numbers
-- 3. Run this in Supabase SQL Editor
-- ============================================================================

-- First, let's check what subjects exist for 6th semester CSE
-- Run this query first to find a valid subject_code:
...
-- ============================================================================
-- STEP 3: Define the subject code to use
-- ============================================================================
-- IMPORTANT: Change this to a valid subject_code that exists in your subjects table
-- Run the SELECT query at the top first to find valid subject codes
DO $$
DECLARE
    v_subject_code VARCHAR(10) := 'CS601';  -- <-- CHANGE THIS to your actual subject code
    v_department VARCHAR(10) := 'CSE';
    v_semester INTEGER := 6;
    v_section VARCHAR(5) := 'A';
    v_period INTEGER := 1;  -- Using period 1 as default since this is daily attendance
BEGIN
    RAISE NOTICE 'Using subject_code: %, department: %, semester: %, section: %', 
                  v_subject_code, v_department, v_semester, v_section;
END $$;

-- ============================================================================
-- STEP 4: Insert attendance records into the main attendance table
-- ============================================================================
-- This will unpivot the data: each day becomes a separate row in attendance table
-- The DB trigger will automatically update attendance_summary and overall_attendance_summary

-- IMPORTANT: Replace 'CS601' below with your actual subject code!
-- You can find valid subject codes by running the SELECT at the top

-- Dec 04, 2025 (Thursday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-04'::date, 1, (dec_04 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();

-- Dec 05, 2025 (Friday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-05'::date, 1, (dec_05 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();

-- Dec 06, 2025 (Saturday - working day apparently)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-06'::date, 1, (dec_06 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
-- Dec 08, 2025 (Monday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-08'::date, 1, (dec_08 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
-- Dec 09, 2025 (Tuesday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-09'::date, 1, (dec_09 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-10'::date, 1, (dec_10 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();

-- Dec 11, 2025 (Thursday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-11'::date, 1, (dec_11 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
-- Dec 12, 2025 (Friday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-12'::date, 1, (dec_12 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-13'::date, 1, (dec_13 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
-- Dec 15, 2025 (Monday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-15'::date, 1, (dec_15 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
-- Dec 16, 2025 (Tuesday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-16'::date, 1, (dec_16 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-17'::date, 1, (dec_17 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();

-- Dec 18, 2025 (Thursday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-18'::date, 1, (dec_18 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
-- Dec 19, 2025 (Friday)
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-19'::date, 1, (dec_19 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
INSERT INTO attendance (registration_no, subject_code, date, period_number, is_present, department, semester, section)
SELECT reg_no, 'CS601', '2025-12-20'::date, 1, (dec_20 = 'P'), 'CSE', 6, 'A'
FROM temp_attendance_import
ON CONFLICT (registration_no, subject_code, date, period_number) DO UPDATE 
SET is_present = EXCLUDED.is_present, marked_at = NOW();
...
-- ============================================================================
SELECT 'Total attendance records inserted:' as info, COUNT(*) as count
FROM attendance 
...
-- ============================================================================
-- DONE! The attendance data should now be visible in the app for 6th semester CSE-A
-- ============================================================================
